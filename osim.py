...
# See reward function on line 601
class OsimModel(object):
    ...
    def __init__(self, model_path, visualize, integrator_accuracy = 5e-5):
        ...
        self.forceSet = self.model.getForceSet()        self.bodySet = self.model.getBodySet()
        ...
        # we will change levels of constants.import math
import numpy as np
import os
from .utils.mygym import convert_to_gym
import gym
...
class ProstheticsEnv(OsimEnv):
    ...
    def reward_round1(self):
        ...
        #reward_upright = - (state_desc["body_vel"]["pelvis"][1])**2 # z direction
        
        # reward for leaning forwards
        if state_desc["body_pos"]["torso"][0]>state_desc["body_pos"]["pelvis"][0]:
            reward_trunk=1
        elif state_desc["body_pos"]["torso"][0]<=state_desc["body_pos"]["pelvis"][0]:
            reward_trunk=-1
        
        # reward for x-velocity
        reward_trunk_velocity = 1- (state_desc["body_vel"]["torso"][0] - 1)**2
        reward_velocity =1- (state_desc["body_vel"]["pelvis"][0] - 1)**2
        
        # reward for not falling
        reward = 1
        return reward_trunk+reward+reward_trunk_velocity 
...
class ProstheticsEnv(OsimEnv):
    ...
    def reward_round1(self):
        ...
        prev_state_desc = self.get_prev_state_desc()
        penalty = 0
        reward = 10 # reward for not falling
        
        if not prev_state_desc:
            return 0
        penalty += np.sum(np.array(self.osim_model.get_activations())**2) * 0.001
        #reward_upright = - (state_desc["body_vel"]["pelvis"][1])**2 # z direction
        reward_trunk = 9.0 - (state_desc["body_vel"]["torso"][0] - 3.0)**2
        reward_velocity = 9.0 - (state_desc["body_vel"]["pelvis"][0] - 3.0)**2
        return reward_trunk+reward_velocity - penalty ## x projection
